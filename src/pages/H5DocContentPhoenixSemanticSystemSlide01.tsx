  ...
  return [
    {
      ...
      numeral: '01',
      title: 'Semantic-to-Rug Compilation Chain',
      blocks: [
        <>
          <div style={infoTagStyle(accentColor, 'tech')}>method appendix</div>
          <p style={paragraphStyle()}>
            这组 appendix 不再解释“页面长什么样”，而是解释系统到底如何把一句模糊输入压成三条可比较的 rug 方向。关键点不是 prompt engineering，而是 semantic compilation：先分辨确定项与含混项，再把含混项展开成几个不同的成立方式。
          </p>
          <h2 style={h2Style(accentColor)}>这条链路真正解决什么问题</h2>
          <p style={paragraphStyle()}>
            用户最常给出的不是完整 brief，而是混合着 mood、偏好、限制和局部想象的句子。系统不能把这类输入直接压成一句 prompt，否则三张图往往只会变成“同一路径上的微调”。第一轮真正要解决的，是把方向空间先展开出来。
          </p>
...
→ 3 generated variants`}</pre>
          ...
          <h2 style={h2Style(accentColor)}>每一步在判断什么</h2>
