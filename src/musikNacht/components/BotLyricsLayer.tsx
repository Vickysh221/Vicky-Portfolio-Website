...
export function BotLyricsLayer({ config }: BotLyricsLayerProps) {
  const { songs, playback, refreshPlayerState } = useScenePlayer()
  ...
  const actualSong = useMemo(
    () => songs.find((song) => song.id === playback.currentSongId) ?? null,
    [songs, playback.currentSongId],
  )
  ...
  }, [isSceneCurrent, playback.progress, playback.status, sceneSong?.id])

  useEffect(() => {
    if (!isSceneCurrent || playback.status !== 'playing') return
  ...
  }, [isSceneCurrent, playback.progress, playback.status])

  useEffect(() => {
    if (!isSceneCurrent || playback.status !== 'playing') return
    const id = window.setInterval(() => { void refreshPlayerState() }, 2500)
    return () => window.clearInterval(id)
  }, [isSceneCurrent, playback.status, refreshPlayerState])

  // ── Fetch lyrics ──
  useEffect(() => {
    let cancelled = false
    async function load() {
      if (!sceneSong?.id) return
      setLyrics([])
      setLyricHint('歌词载入中…')
      try {
        const res  = await fetch(`/api/library/lyrics?songId=${sceneSong.id}`)
  ...
  return (
    ...
    >
      ...
      {activeLine ? (
        <div key={`${sceneSong.id}-${activeIdx}`} style={{ display: 'grid', gap: 7 }}>
          <div
            style={{
