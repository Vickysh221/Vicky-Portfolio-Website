...
export function BotPlayerPanel({ config }: BotPlayerPanelProps) {
  const { songs, playback, playSelectedSong, pausePlayback, playerBusy } = useScenePlayer()

  const actualSong = songs.find((song) => song.id === playback.currentSongId) ?? null
  const sceneSong =
    config.resolvedSong ??
  ...
  const isCurrentBot =
    sceneSong !== null &&
    (playback.currentSongId === sceneSong.id || actualSong?.originalId === config.originalSongId)
  const isPlaying = isCurrentBot && playback.status === 'playing'

  const handleToggle = async () => {
    if (!sceneSong) return
    if (playerBusy) return
    if (isPlaying) await pausePlayback()
    else await playSelectedSong(sceneSong?.id)
  }

  const displaySong = sceneSong
  const displayTitle = displaySong?.name ?? config.songTitle
  const displayArtist = displaySong?.artistNames?.join(', ') ?? config.artist
  const coverUrl = displaySong?.coverImgUrl ?? null

  const accent = config.colorAccent

  const dur = isCurrentBot
    ? (playback.duration ?? (displaySong?.duration ? displaySong.duration / 1000 : 0))
    : (displaySong?.duration ? displaySong.duration / 1000 : 0)
  const prog = isCurrentBot ? (playback.progress ?? 0) : 0
  ...
  return (
    ...
    >
      ...
      <button
        ...
        disabled={playerBusy || !sceneSong}
        title={!sceneSong ? '当前场景暂无可播放歌曲映射' : isPlaying ? '暂停' : '播放'}
        style={{
          width: 40,
          height: 40,
          borderRadius: '50%',
          ...
          color: accent,
          fontSize: 16,
          cursor: playerBusy ? 'wait' : !sceneSong ? 'not-allowed' : 'pointer',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          flexShrink: 0,
